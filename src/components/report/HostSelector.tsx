        ...
        <div className="relative border border-slate-100 rounded-xl p-4 pt-8 bg-white shadow-sm transition-all hover:shadow-md h-full flex flex-col">
            ...
            <div className="space-y-2 mb-3">
                <div className="relative group">
                    ...
                    <input
                        className="w-full pl-8 pr-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400 placeholder:font-medium shadow-inner"
                        placeholder="Search..."
                        value={searchTerm}
                ...
                <div className="flex gap-1.5">
                    <button onClick={onExpandAll} className="flex-1 py-1.5 bg-slate-100 text-slate-700 rounded-xl text-[9px] font-bold capitalize tracking-wider hover:bg-slate-200 transition-colors border border-slate-200/50">Expand</button>
                    <button onClick={onCollapseAll} className="flex-1 py-1.5 bg-slate-100 text-slate-700 rounded-xl text-[9px] font-bold capitalize tracking-wider hover:bg-slate-200 transition-colors border border-slate-200/50">Reset</button>
                </div>
            </div>

            <div className="flex-1 space-y-1 overflow-y-auto pr-1 custom-scrollbar min-h-[200px] max-h-[400px]">
                {groups.length === 0 ? (
                    <div className="py-4 text-center text-slate-400 font-medium text-xs italic">No match.</div>
                ) : groups.map((g: any) => {
                    const isExpanded = expandedGroups.includes(g.hostgroup);
                    const isGroupSelected = selectedGroups.includes(g.hostgroup);
                    
                    return (
                        <div key={g.hostgroup} className="border border-slate-100 rounded-xl bg-white overflow-hidden transition-all duration-200 hover:border-blue-100">
                            <div
                                className={`flex items-center justify-between p-2 cursor-pointer transition-all ${isGroupSelected ? 'bg-blue-600 text-white' : 'hover:bg-slate-50'}`}
                                onClick={() => onToggleGroup(g.hostgroup)}
                            >
                                <div className="flex items-center gap-1.5 min-w-0">
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); onToggleExpand(g.hostgroup); }}
                                        className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${isGroupSelected ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-blue-600'}`}
                                    >
                                    ...
                                    {g.hostnames.map((h: any) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between gap-1 p-2 pl-6 rounded-xl text-[9px] font-semibold transition-all ${isSelected ? 'bg-white text-blue-700 border border-blue-100 shadow-sm' : 'text-slate-500 hover:bg-white hover:text-slate-900'}`}
                                            >
