        ...
        <div className="flex items-center gap-10">
          <div className="flex items-center gap-3 group cursor-pointer">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-xs font-black text-muted capitalize tracking-widest">Infrastructure</span>
            </div>
          </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-xs font-black capitalize tracking-widest transition-all duration-300 ${isActive ? 'bg-blue-700 text-white shadow-lg shadow-blue-700/20' : 'text-muted hover:bg-blue-50 hover:text-blue-700'}`}
                >
                ...
                >
                  <div className="p-6 bg-blue-50/50 rounded-[2rem] mb-4 space-y-4 flex flex-col items-center justify-center text-center border border-blue-100">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-black text-blue-400 capitalize tracking-widest">Operator</p>
                      <p className="text-base font-black text-slate-900 leading-tight">{session?.user?.name || 'Guest User'}</p>
                      <div className="inline-block px-4 py-1 rounded-full bg-blue-700 text-white text-[9px] font-black capitalize tracking-widest border border-blue-800 shadow-md shadow-blue-200 mt-2">
                        {userRole || 'Guest'}
                      </div>
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-5 py-4 text-red-600 hover:bg-red-50 rounded-xl transition-all font-black capitalize tracking-widest text-xs mt-2 border border-transparent hover:border-red-100"
                    >
              ...
              {navItems.map(item => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-4 px-6 py-5 rounded-xl text-sm font-black capitalize tracking-widest transition-all ${isActive ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
                  >
