...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = isMem 
        ? await MetricService.getMemMonthly(Number(user.id), user.role, hostgroup, Number(hostnameId), month, year)
        : await MetricService.getCpuMonthly(Number(user.id), user.role, hostgroup, Number(hostnameId), month, year);
