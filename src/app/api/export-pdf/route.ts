...
// Read Highcharts scripts from local node_modules for speed and offline support
const getHighchartsScripts = () => {
  try {
...
const generateHTML = (payload: ReportPayload, pageMap: Record<string, number> = {}, logoBase64: string = '', totalLogicalPages: number = 0): { html: string, chartsToRender: any[] } => {
  ...
  // Data for client-side rendering of Highcharts
  const chartsToRender: any[] = [];
  const hcScripts = getHighchartsScripts();
    ...
    return chartPairs.map((pair, pi) => {
      ...
      return `
        ...
        chartsToRender.push({
          ...
          hostMem: chart.hostMem,
          totalAvg: chart.totalAvg,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { html, chartsToRender } = generateHTML(payload, {}, logoBase64);
    await page.setContent(html, { waitUntil: 'domcontentloaded', timeout: 300000 } as any);

    // Chunked data transfer to prevent CDP buffer overflow and Chromium crashes
    const CHUNK_SIZE = 50; // Render 50 charts at a time
    for (let i = 0; i < chartsToRender.length; i += CHUNK_SIZE) {
      const chunk = chartsToRender.slice(i, i + CHUNK_SIZE);
      await page.evaluate((data) => {
        (window as any).accumulatedCharts = ((window as any).accumulatedCharts || []).concat(data);
      }, chunk);
    }

    await page.evaluate(() => {
      const renderBatch = async (charts: any[]) => {
        await Promise.all(charts.map(async (item: any) => {
          try {
      ...
      if ((window as any).accumulatedCharts) {
        renderBatch((window as any).accumulatedCharts);
      }
    });
    // Give breathing room based on chart count
    await new Promise(r => setTimeout(r, Math.min(15000, chartsToRender.length * 50)));
    ...
    const finalRender = generateHTML(payload, result.map, logoBase64, result.totalPhysicalPages - offset);
    await page.setContent(finalRender.html, { waitUntil: 'domcontentloaded', timeout: 300000 } as any);

    // Reset and repopulate for final render
    await page.evaluate(() => { (window as any).accumulatedCharts = []; });
    for (let i = 0; i < finalRender.chartsToRender.length; i += CHUNK_SIZE) {
      const chunk = finalRender.chartsToRender.slice(i, i + CHUNK_SIZE);
      await page.evaluate((data) => {
        (window as any).accumulatedCharts = ((window as any).accumulatedCharts || []).concat(data);
      }, chunk);
    }
    ...
    await new Promise(r => setTimeout(r, Math.min(15000, finalRender.chartsToRender.length * 50)));
