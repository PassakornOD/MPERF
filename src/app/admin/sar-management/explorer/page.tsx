      ...
      <div className="modern-card p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
          ...
          <div className="flex gap-2 h-[46px]">
            <button onClick={() => handleQuery(1)} className="flex-1 bg-slate-900 hover:bg-black text-white rounded-xl px-6 py-2 text-xs capitalize  transition-all flex items-center justify-center gap-2 shadow-xl shadow-slate-200">
                {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin"/> : <Search className="w-3.5 h-3.5" />} Fetch
            </button>
            <button onClick={() => setIsConfirmOpen(true)} className="flex-none bg-red-50 hover:bg-red-600 text-red-600 hover:text-white rounded-xl px-4 py-2 text-xs capitalize  transition-all border border-red-100 shadow-sm">
                Purge
            </button>
          </div>
        </div>
