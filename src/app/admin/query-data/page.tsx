...
const QueryDataPage = () => {
  ...
  return (
    <div className="space-y-10 animate-ease-in">
        <div className="modern-card p-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
            <div className="space-y-2 lg:col-span-1">
                <label className="text-xs text-muted-foreground capitalize ml-1">Database Table</label>
                <input 
                  placeholder="e.g. BSS:u"
                  className="w-full bg-background border border-border rounded-xl py-3 px-4 text-xs capitalize tracking-tight outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-200 transition-all shadow-inner"
                  value={formData.table}
                  onChange={(e) => setFormData({...formData, table: e.target.value})}
                />
            </div>
            <div className="space-y-2">
                <label className="text-xs text-muted-foreground capitalize ml-1">Year</label>
                <input 
                  placeholder="YYYY"
                  className="w-full bg-background border border-border rounded-xl py-3 px-4 text-xs outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-200 transition-all shadow-inner"
                  value={formData.year}
                  onChange={(e) => setFormData({...formData, year: e.target.value})}
                />
            </div>
            <div className="space-y-2">
                <label className="text-xs text-muted-foreground capitalize ml-1">Month</label>
                <input 
                  placeholder="MM"
                  className="w-full bg-background border border-border rounded-xl py-3 px-4 text-xs outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-200 transition-all shadow-inner"
            ...
            <div className="space-y-2">
                <label className="text-xs text-muted-foreground capitalize ml-1">Day</label>
                <input 
                  placeholder="DD"
                  className="w-full bg-background border border-border rounded-xl py-3 px-4 text-xs outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-200 transition-all shadow-inner"
                  value={formData.day}
        ...
        {queryData.length > 0 && (
          <div className="bg-card rounded-[2rem] border border-border shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="px-8 py-5 border-b border-slate-50 bg-background/30 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <TableIcon className="w-4 h-4 text-blue-600" />
                    <span className="text-xs text-foreground capitalize">Query Result Set</span>
                </div>
            ...
            <div className="overflow-x-auto custom-scrollbar border border-border rounded-xl shadow-inner bg-background/20 mx-8 my-8">
            ...
            <div className="p-4 bg-background/30 border-t border-slate-50 text-center">
