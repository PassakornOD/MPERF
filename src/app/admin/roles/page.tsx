    ...
    <Block title="Authorization Framework" subtitle="Global role-to-resource mapping governance">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 animate-ease-in">
        <div className="lg:col-span-1 space-y-6">
          <label className="text-xs text-muted-foreground capitalize  ml-2 block">System Roles</label>
          <div className="bg-card p-2 rounded-[2rem] border border-border shadow-sm space-y-1">
          {roles.map(r => (
            <button 
              key={r.role_id}
              onClick={() => handleRoleSelect(r.role_id)}
              className={`block w-full text-left px-5 py-4 rounded-xl text-xs capitalize tracking-tight transition-all duration-300 ${selectedRole === r.role_id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20 translate-x-1' : 'text-muted-foreground hover:bg-background hover:text-foreground'}`}
            >
              {r.role_name}
            </button>
          ))}
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-card p-10 rounded-[2rem] border border-border shadow-sm min-h-[400px] flex flex-col">
            {!selectedRole ? (
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-10 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar p-1">
                        {hostgroups.map(hg => {
                            ...
                            return (
                                <label key={hg.hostgroup_id} className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all group ${isChecked ? 'bg-blue-50 border-blue-200 shadow-sm' : 'bg-card border-border hover:border-border shadow-inner'}`}>
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className={`w-2 h-2 rounded-full shrink-0 ${isChecked ? 'bg-blue-600' : 'bg-slate-200'}`}></div>
                                        <span className={`text-[11px] capitalize tracking-tight truncate ${isChecked ? 'text-blue-700' : 'text-muted-foreground group-hover:text-slate-700'}`}>{hg.hostgroup}</span>
                                    </div>
                                    <input 
                                        type="checkbox" 
                                        checked={isChecked}
                                        onChange={() => toggleHostgroup(hg.hostgroup_id)}
                                        className="w-4 h-4 rounded-xl border-border text-blue-600 focus:ring-blue-500 transition-all cursor-pointer"
                                    />
                                </label>
                            )
                        })}
