                ...
                <div className="p-8 space-y-8">
                    <div className="flex justify-between items-center border-b border-slate-50 pb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-sm text-slate-800 capitalize">Resource Permissions</h3>
                                <p className="text-xs text-slate-400 capitalize mt-0.5">Map server hostgroups to access levels</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full text-xs text-left border-collapse">
                            <thead>
                                <tr className="bg-white border-b border-slate-100">
                                    <th className="px-8 py-5 text-[9px] text-slate-400 capitalize">Access Group</th>
                                    <th className="px-8 py-5 text-[9px] text-slate-400 capitalize">Associations</th>
                                    <th className="px-8 py-5 text-[9px] text-slate-400 capitalize">Secured Assets</th>
                                    <th className="px-8 py-5 text-[9px] text-slate-400 capitalize text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100/50 bg-white">
                                {data.pgs.map((pg: any) => {
                                    ...
                                    return (
                                        <tr key={pg.pg_id} className="group/row hover:bg-blue-50/40 transition-all duration-200 border-b border-slate-100/30 last:border-0">
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <p className="text-slate-900 text-sm capitalize tracking-tight group-hover/row:text-blue-700 transition-colors">{pg.pg_name}</p>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5">
                                                <div className="flex flex-wrap gap-2 max-w-[240px]">
                                                    {data.ugPgs.filter((m: any) => m.pg_id === pg.pg_id).slice(0, 2).map((m: any) => {
                                                        ...
                                                        return ug ? <span key={m.ug_id} className="bg-white border border-slate-100 text-slate-500 px-2.5 py-1 rounded-xl font-bold text-[9px] shadow-sm whitespace-nowrap">{ug.ug_name}</span> : null;
                                                    })}
                                            ...
                                            <td className="px-8 py-5 text-right">
                                                <div className="relative flex justify-end">
                                                    <button
                                                        ...
                                                        className={`p-2.5 rounded-xl transition-all border ${activeMenu === pg.pg_id ? 'bg-white shadow-md text-slate-900 border-slate-200' : 'text-slate-300 hover:text-slate-600 border-transparent hover:bg-white hover:shadow-sm hover:border-slate-100'}`}
                                                    >
                        ...
                        <div className="mt-12 border-t border-slate-100 pt-10">
                            <div className="flex justify-between items-center mb-6 px-2">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-sm text-slate-800 capitalize">Unprotected Assets</h3>
                            ...
                            <div className="flex flex-wrap gap-3 p-6 bg-slate-50/30 rounded-[2rem] border border-slate-100 shadow-inner">
                        ...
                        label: 'General', content: (
                            <div className="space-y-8 pt-2">
                                <div className="flex gap-4 p-6 bg-slate-50/50 rounded-xl items-center border border-slate-100 shadow-inner">
                                    ...
                                    <div>
                                        <p className="text-xs text-slate-400 capitalize mb-0.5">Configuration</p>
                                        <p className="text-xl text-slate-900 tracking-tight">{editingPg?.pg_name}</p>
                                    </div>
                                </div>
                        ...
                        label: 'Associated', content: (
                            <div className="space-y-4 pt-2">
                                {editingPg && data.ugPgs && data.ugPgs.filter((m: any) => m.pg_id === editingPg.pg_id).length > 0 ? data.ugPgs.filter((m: any) => m.pg_id === editingPg.pg_id).map((m: any) => {
                                    ...
                                    return (
                                        <div key={m.ug_id} className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white hover:border-blue-100 transition-all">
                                            <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => setExpandedPgId(isExpanded ? null : `ug-${ug.ug_id}`)}>
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <span className="text-xs text-slate-800 tracking-tight">{ug.ug_name}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                    ...
                                    {editFields.hostgroup_ids.length > 0 ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {editFields.hostgroup_ids.map(hgId => {
                                                ...
                                                return hg ? (
                                                    <div key={hgId} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center justify-between shadow-sm group/item hover:border-blue-200 transition-all">
                                                        <div className="flex items-center gap-3 overflow-hidden">
                                                            ...
                                                            <div className="flex flex-col truncate">
                                                                ...
                                                                <span className="text-[8px] text-slate-400 capitalize">{hostCount} Hosts</span>
                                                            </div>
                                                        </div>
                                                ...
                                                {availableHostgroups.map(hg => {
                                                    ...
                                                    return (
                                                        <button key={hg.hostgroup_id} onClick={async () => {
                                                            ...
                                                            await axios.post('/api/admin/permission-groups/update', { pg_id: editingPg.pg_id, ...editFields, hostgroup_ids: nextHgs });
                                                            fetchData();
                                                        }} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center gap-3 shadow-sm hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all text-left group/btn">
                                                            <div className="bg-slate-50 p-1.5 rounded-xl group-hover/btn:bg-blue-500 transition-colors shadow-inner"><Plus className="w-3.5 h-3.5 text-blue-600 group-hover/btn:text-white" /></div>
                                                            <div className="flex flex-col truncate">
