  ...
  return (
    <div className="space-y-12 animate-ease-in">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl  text-foreground capitalize tracking-tight">Dashboard Overview</h1>
        <p className="text-sm font-medium text-muted-foreground">Quick access to infrastructure monitoring and reporting tools</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <Link key={item.title} href={item.href} className="modern-card p-8 flex flex-col gap-5 group">
            ...
            <div>
              <h3 className="text-lg  text-foreground capitalize tracking-tight">{item.title}</h3>
      ...
      <Block title="Hostgroup Explorer" subtitle="Aggregate hostnames by group selection">
            ...
            return (
              <button
                ...
                className={`px-6 py-2.5 rounded-xl text-xs  capitalize  transition-all duration-300 border ${isActive
                  ...
                  : 'bg-card border-border text-muted-foreground hover:border-blue-200 hover:text-blue-600 shadow-sm'
                  }`}
              >
                {g.hostgroup}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {selectedGroups.map((group: any) => (
            <div key={group.hostgroup_id} className="bg-background/50 rounded-xl border border-border p-6 flex flex-col gap-4 animate-ease-in">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <h4 className=" text-foreground capitalize tracking-tight text-sm flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-blue-500" />
                  {group.hostgroup}
                </h4>
                <span className="text-xs  text-muted-foreground bg-card px-2 py-0.5 rounded-xl border border-border">{group.hostnames.length} Hosts</span>
              </div>
              <ul className="grid grid-cols-1 gap-2">
                {group.hostnames.map((h: any) => (
                  <li key={h.hostname_id} className="text-xs text-muted-foreground flex items-center gap-2 hover:text-blue-600 transition-colors cursor-default group/item">
