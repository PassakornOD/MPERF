                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    {canCreate && (
                        <div className="flex items-center gap-2">
                            <button onClick={() => { setEditingGroup(null); setNewGroup({ hostgroup: '', owner: '', pg_id: '' }); setIsGroupModalOpen(true); }} className="bg-blue-600 text-white px-6 py-3 rounded-xl text-xs capitalize  hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2">
                                <Plus className="w-4 h-4" /> Hostgroup
                            </button>
                            <button onClick={() => { setEditingHost(null); setHostForm({ hostname: '', hostgroup_id: '', System: '', Location: '', IP: '', Model: '', CPU: '', Disk: '', OS: '', Serial: '', MA: '', mem: '', Pagesize: '4096' }); setIsHostModalOpen(true); }} className="bg-slate-900 text-white px-6 py-3 rounded-xl text-xs capitalize  hover:bg-black transition-all shadow-lg shadow-slate-200 flex items-center gap-2">
                                <Plus className="w-4 h-4" /> Hostname
                            </button>
                        </div>
                    )}
                </div>
            ...
            <Modal isOpen={isGroupModalOpen} onClose={() => { setIsGroupModalOpen(false); setEditingGroup(null); }} title={editingGroup ? `Modify hostgroup: ${editingGroup.hostgroup}` : "Create New Hostgroup"} maxWidth="max-w-md">
                <div className="space-y-6 pt-2">
                    ...
                    <button
                        ...
                        className="w-full bg-blue-600 text-white py-4 rounded-xl text-xs capitalize  hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 disabled:opacity-50 mt-4 flex items-center justify-center group"
                    >
            ...
            <Modal isOpen={isHostModalOpen} onClose={() => { setIsHostModalOpen(false); setEditingHost(null); }} title={editingHost ? `Update Hostname: ${editingHost.hostname}` : "Create New Hostname"} maxWidth="max-w-2xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                    ...
                    <button
                        ...
                        className="md:col-span-2 bg-slate-900 text-white py-4 rounded-xl text-xs capitalize  hover:bg-black transition-all shadow-xl shadow-slate-200 disabled:opacity-50 mt-4 flex items-center justify-center group"
                    >
